...
Start by selecting the __Patch (Manifest)__ stage from the stage selector:
...
Like the [Deploy (Manifest) stage](/guides/user/kubernetes-v2/deploy-manifest/index.md#specify-manifests-dynamically), you can also reference an artifact as the source if you are storing your patch content externally. The artifact must be a text file containing the patch content.

You can also setup the pipeline to trigger based on changes to the patch content:

* [Consuming GitHub Artifacts](/guides/user/triggers/github)
* [Consuming GCS Artifacts](/guides/user/triggers/gcs)

Assuming you have declared an expected artifact upstream to your Patch
manifest stage, you can reference it in the Patch configuration:
...
The patch stage also allows you to [override artifacts](/guides/user/kubernetes-v2/deploy-manifest/index.md#override-artifacts) like in the deploy manifest stage.
...
You can also optionally specify the following options:
...
* __Merge Strategy__
  ...
  Three merge strategies are supported for the patch operation:
