...
## I'm unable to load the Applications screen
 ...
 * The following error message is displayed: <em>Error fetching applications</em> (release 1.9 and later)

 The most common cause of this error is that your browser cannot communicate with your Gate endpoint.
 ...
 * If you are accessing Spinnaker via a custom URL, ensure that you have set `override-base-url`
   for both the UI (Deck) and API (Gate) services, as described in the question below.  Failure to do
   this will lead to your browser rejecting requests to Gate due to their failing the CORS preflight
   check.
