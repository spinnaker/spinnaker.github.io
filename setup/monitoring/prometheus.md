...
Alternitavely, you can deploy Prometheus to use a Gateway server wherein
...
## Configure the Spinnaker Monitoring Daemon for Prometheus
  ...
  2. Run the client-side configuration script.
     ...
     * The `--client_host` is an optional parameter that configures the
       ...
       than the default `localhost`. The host `` adds all of them. This
       is illustrated for simplicity. By exposing a network port other
       than local_host, the server will be able to poll the client from
...
on the type of dashboard. As a rule of thumb, the following are avaialable:
   ...
   * There is an application-oriented dashboard that lets you select
     ...
     show up there, then you may need to wait a little while longer until
     the application becomes visible. In the meantime, you can use Grafana
     to hack the dashboard to add your desired value into the list of
     choices or create your own custom charts in the meantime.
...
The Grafana dashboards are typically "idelta" or "rates" at 1m intervals.
